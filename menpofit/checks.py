...
def check_max_iters(max_iters, n_scales):
    ...
    return np.require(max_iters, dtype=int)
...
def check_sampling(sampling, n_scales):
    ...
    if (isinstance(sampling, (list, tuple)) and
        np.alltrue([isinstance(s, (np.ndarray, int)) or sampling is None
                    for s in sampling])):
        if len(sampling) == 1:
    ...
    elif isinstance(sampling, (np.ndarray, int)) or sampling is None:
        return [sampling] * n_scales
    else:
